...
namespace PetVerse.Controllers
{
    ...
    public class PostsController : ControllerBase
    {
        ...
        [HttpPost("user/event")]
        public async Task<IActionResult> CreateEventPost(CreateEventPostDTO dto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest();
            ...
            try
            {
                result = await _postService.CreateEventPostAsync(userId, dto);
            }
            catch (ValidationException e)
            {
                return BadRequest(e.Message);
            ...
            var responseDTO = new EventPostResponseDTO
            {
                Id = result.Id,
                Title = result.Title,
                Body = result.Body,
            ...
            return CreatedAtAction(nameof(GetEventPostById), new { id = result.Id }, responseDTO);
        }

        [HttpGet("user/event/{id}")]
        public async Task<ActionResult<EventPostResponseDTO>> GetEventPostById(int id)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest();
            ...
            var post = await _postService.GetEventPostByIdAsync(id);
            if (post == null) return NotFound();
